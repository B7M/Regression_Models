...
plot(x, y, type = "n", frame = FALSE, xlab = "x", ylab = "y")
abline(lm(y ~ x), lwd = 2)
abline(h = mean(y[1 : (n/2)]), lwd = 3,col="red")
abline(h = mean(y[(n/2 + 1) : n]), lwd = 3,col="red")
fit <- lm(y ~ x + t)
abline(coef(fit)[1], coef(fit)[2], lwd = 3,col="green")
abline(coef(fit)[1] + coef(fit)[3], coef(fit)[2], lwd = 3,col="green")

# Add histogram bins
hist(x, breaks = 10, add = TRUE, col = rgb(0, 0, 0, 0.2))

# Add points with different colors for each group
points(x[1 : (n/2)], y[1 : (n/2)], pch = 21, col = "black", bg = "lightblue", cex = 2)
points(x[(n/2 + 1) : n], y[(n/2 + 1) : n], pch = 21, col = "black", bg = "salmon", cex = 2)

# Calculate and add the vertical distance label between two horizontal lines
distance_label <- c(1, 2)

# Add two text labels to the y-axis using mtext
mtext(side = 2, text = c("Marginal effect", "Linear effect"), at = c(1.5, .35), line = c(2, 2), col = c("red","green"), cex = 1)
```
...
sigma <- 0.01
y <- beta0 + x1 * beta1 + tau * x2 + rnorm(n, sd = sigma)
plot(x2, y, pch = 21, col = "black",lwd=2,
     xlab = "x2", ylab = "y", main = "Variation of y with respect to x2")
```
...
```{r, echo = F, eval = T}
plot_fulcrum_arrow <- function(length, position) {
  plot(c(0, 0), c(0, 1), type = 'n', xlab = "", ylab = "", xlim = c(-length, length-3), ylim = c(0, 1))

  segments(-length, 0.5, length-3.5, 0.5, col = "brown", lwd = 3)
  
  points(position-1, 0.435, col = "green", pch = 17, cex = 4)
  points(position-2, 0.58, col = "black", pch = 15, cex = 8)

  arrows(x0 = position + 1, y0 = 0.7, x1 = position + 1, y1 = 0.5, col = "red", length = 0.2,lwd=5)
  arrows(x0 = position + 3, y0 = 0.7, x1 = position + 3, y1 = 0.5, col = "blue", length = 0.2,lwd=5)
}

plot_fulcrum_arrow(length = 5, position = -2) 
```
...
The next example is by Stefanski et.al. that shows why we do residual plots. Basically, the reason we look at residual plots is because they zoom on potential problems with our model. 
```{r, echo = T, eval = T}
dat <- read.table('http://www4.stat.ncsu.edu/~stefanski/NSF_Supported/Hidden_Images/orly_owl_files/orly_owl_Lin_4p_5_flat.txt', header = FALSE)
pairs(dat)
```
The pairwise scatter plots show nothing interesting other than a lot of over plotting. 

```{r, echo = T, eval = T}
summary(lm(V1 ~ . -1, data = dat))$coef
```

In this model every single P values is highly significant for the coefficients. In multi variable examples because you can't plot the residuals like in linear regression and you need to pick a number of the X with the most common ways to plot the residuals versus the fitted values. Doing so will show you a clever little picture that comes out. 
```{r, echo = T, eval = T}
fit <- lm(V1 ~ . - 1, data = dat); 
plot(predict(fit), resid(fit), pch = '.')
```

Without looking at the residuals we wouldn't have seen anything. But looking at the residuals has zoomed in on the clear aspect of poor model fit with a clear systematic pattern in the residuals that we missed. 
